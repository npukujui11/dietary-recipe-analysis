...
% 构建目标函数，最大化总的蛋白质氨基酸评分
AAS_breakfast = (x_breakfast' * breakfast_essential_aa) / (x_breakfast' * breakfast_reference_protein_aa) * 100;
AAS_lunch = (x_lunch' * lunch_essential_aa) / (x_lunch' * lunch_reference_protein_aa) * 100;
AAS_dinner = (x_dinner' * dinner_essential_aa) / (x_dinner' * dinner_reference_protein_aa) * 100;
AAS_total = AAS_breakfast + AAS_lunch + AAS_dinner;
prob = optimproblem('Objective', -AAS_total); % 最大化AAS, 所以使用负号
...
% 早餐能量分配
prob.Constraints.energy_breakfast_min = sum(x_breakfast .* breakfast_energy) >= 0.25 * sum([x_breakfast; x_lunch; x_dinner] .* [breakfast_energy; lunch_energy; dinner_energy]);
prob.Constraints.energy_breakfast_max = sum(x_breakfast .* breakfast_energy) <= 0.35 * sum([x_breakfast; x_lunch; x_dinner] .* [breakfast_energy; lunch_energy; dinner_energy]);

% 中餐能量分配
prob.Constraints.energy_lunch_min = sum(x_lunch .* lunch_energy) >= 0.30 * sum([x_breakfast; x_lunch; x_dinner] .* [breakfast_energy; lunch_energy; dinner_energy]);
prob.Constraints.energy_lunch_max = sum(x_lunch .* lunch_energy) <= 0.40 * sum([x_breakfast; x_lunch; x_dinner] .* [breakfast_energy; lunch_energy; dinner_energy]);

% 晚餐能量分配
prob.Constraints.energy_dinner_min = sum(x_dinner .* dinner_energy) >= 0.30 * sum([x_breakfast; x_lunch; x_dinner] .* [breakfast_energy; lunch_energy; dinner_energy]);
prob.Constraints.energy_dinner_max = sum(x_dinner .* dinner_energy) <= 0.40 * sum([x_breakfast; x_lunch; x_dinner] .* [breakfast_energy; lunch_energy; dinner_energy]);
